...
namespace CleanArchitecture.Identity.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthResponse> Login(AuthRequest request)
        {
            ...
            {
                Id = user.Id,
                Token = new JwtSecurityTokenHandler().WriteToken(token), //HANDLER ES UN MANEJADOR DE EVENTOS QUE RESPONDEN A LOS EVENTOS QUE SE PRODUCCEN SOBRE EL
                Email = user.Email,
        ...
        private async Task<JwtSecurityToken> GenerateToken(ApplicationUser user)
        {
            ...
            var roles = await _userManager.GetRolesAsync(user);//tiene que ser parseada a tipos claim para poder insertar
            ...
            var claims = new[]
            {
                new Claim(JwtRegisteredClaimNames.Sub, user.UserName), //"username" en ligar delñ jwtregistered
